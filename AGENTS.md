...
- 导出结果工作簿包含两个 sheet：`筛选结果` 使用原始 Excel 列名和原始单元格数据展示命中行，并追加 `得分`、`与基差差距`；`识别异常` 只展示真正需要处理的问题：缺少必需字段、基差无法解析、数据值未命中值别名规则等，并尽量附带原始行数据。未被列名规则映射的普通原始列不再视为异常（它们已作为原始列保留在 `筛选结果` 中），避免备注/产地等列把异常表刷屏。即使没有命中行也要写出空结果工作簿，便于用户确认本次筛选已执行并触发重名递增。
