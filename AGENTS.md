...
- 规则维护页采用撑满页面的左右布局：左侧菜单包含“列名规则”“数据规则 / 值别名”“数据规则 / 评分与过滤区间”，右侧展示当前菜单对应的数据，菜单和内容区域各自内部滚动。数据规则中的“值别名”要与“评分与过滤区间”分开维护；值别名按输出值归组并单列展示；评分区间和过滤区间必须选择同一字段值别名列表中的“输出值”，不能留空或自由填写；关键词过滤命中指定字段文本后保留该行。
